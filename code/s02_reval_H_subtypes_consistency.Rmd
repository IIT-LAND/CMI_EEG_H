...
rs_data = tidy_data %>% 
  ...
  select(subid, rsopen_subgroup_glob, rsclosed_subgroup_glob, age, fiq, assq_total, scq_total, rbs_total, srs_total_T) %>%
  distinct()
...
# save to file
write.csv(rs_data_summ, file = file.path(resultpath,"02_subtype_consistency_summ_stats.csv"))
```
...
# save to file
write.csv(output_res, file = file.path(resultpath,"02_subtype_consistency_lm.csv"))
...
rs_data_2plot = tidy_data %>% 
  filter(dx1_short == "ASD" & task == "RestingState" & sex == "Male" & exclude == "No") %>% 
...
ggsave(filename = file.path(plotpath,"02_subtype_consistency_alluvialplot.pdf"))
