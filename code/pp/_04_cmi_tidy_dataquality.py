# _04_cmi_tidy_dataquality.py
"""
_04_cmi_tidy_dataquality.py

Put all subjects data quality metrics into a tidy csv file.

Example usage:

# Run on server and generate scripts only
python _04_cmi_tidy_dataquality.py server script

# Run on laptop and generate scripts only
python _04_cmi_tidy_dataquality.py laptop script

# Run on server and run batches
python _04_cmi_tidy_dataquality.py server batch

# Run on laptop and run batches
python _04_cmi_tidy_dataquality.py laptop batch
...
metric2use = "dataquality"
main_script = "cmi_tidy_dataquality"
...
for sub in sublist:
    ...
    for task in flist:
        ...
        taskpath = "%s/%s" % (preproc_subpath, task)
        file2lookfor = "%s/%s_%s_preproc_dataQuality.csv" %(taskpath,sub,task)
...
for task in task_names:
    ...
    script.append("")
    r_code2run = "source('%s/cmi_tidy_dataquality.R'); task = '%s'; sublist = read.delim('%s',header=FALSE); masterfile = '%s'; result_df = cmi_tidy_dataquality(sublist,task, masterfile); quit(save = 'no', status = 0, runLast = FALSE);" % (codedir,task, sublist2save[task],masterfiles[task])
